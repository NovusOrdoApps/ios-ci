...
**Screenshot UPLOAD bypasses `deliver`.** Like IAP and availability, the `update_screenshots` lane drives `Spaceship::ConnectAPI` directly instead of `deliver`. `deliver`'s screenshot path (`upload_screenshots.rb`) uploads with `wait_for_processing:false`, then decides "already uploaded?" / "upload succeeded?" by matching the local file's MD5 against ASC's `sourceFileChecksum` — which ASC returns null while it's still asynchronously processing the asset. The just-uploaded file matches nothing → `deliver` thinks it "is missing", retries, and uploads the **same image again** as a second row that is never reconciled; `sort_screenshots` then renders the duplicates adjacent (two identical screenshots on the store). The locales hit are whichever were uploaded last (least processing head-start). `update_screenshots` removes the race: it tracks the `AppScreenshot` IDs it creates (never `sourceFileChecksum`), uploads sequentially, and blocks on each asset reaching the `COMPLETE` delivery state before the next. It is **full-locale replace**: when `skip_screenshots:false`, it deletes every existing screenshot across all device families for each repo-provided locale (and drops sets the repo no longer provides) before uploading exactly what's on disk. Requires an editable version, so `update_metadata` (which always passes `skip_screenshots:true` to `deliver` now) runs first to open one; `ios-metadata.yml` invokes `update_screenshots` as a separate step gated on `inputs.skip_screenshots == false`.
